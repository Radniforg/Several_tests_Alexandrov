...
def database_search_test_1(dbname='dostavista', user='postgres', password='', host='localhost', text=0):
    ...
    while start_date <= end_date:
        if start_date in date_check.keys():
            ...
            result.update({start_date: {'client_payment': date_check[start_date][0],
                                        'revenue': revenue,
                                        'AOR': revenue/date_check[start_date][2]}})
        else:
            result.update({start_date: {'client_payment': 0,
                                        'revenue': 0,
                                        'AOR': 0}})
        start_date += delta
    if text != 0:
        with open('report_test_1.txt', 'w', encoding='UTF-8') as port:
            for key in sorted(result.keys()):
                port.write(f'Date - {key}\n'
                           f'client payments - {result[key]["client_payment"]}\n'
                           f'revenue - {result[key]["revenue"]}\n'
                           f'AOR - {result[key]["AOR"]}\n\n')
    return result
